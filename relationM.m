...
X3 = awgn(A3*S,SNR,'measured');%接收信号
...
for a = 1:length(theta1)
    ...
    for b = 0:M3-1
        AA3(:,b+1) = exp(-1*1i*2*pi*d*sin(theta1(a)/180*pi)/lamda*b); %注：在matlab中三角函数计算使用地是弧度值
    end
    AA3 = AA3.';
    P3 = AA3'*En3*En3'*AA3;
...
for a = 1:length(theta1)
    ...
    for b = 0:M2-1
        AA2(:,b+1) = exp(-1*1i*2*pi*d*sin(theta1(a)/180*pi)/lamda*b); %注：在matlab中三角函数计算使用地是弧度值
    end
    AA2 = AA2.';
    P2 = AA2'*En2*En2'*AA2;
