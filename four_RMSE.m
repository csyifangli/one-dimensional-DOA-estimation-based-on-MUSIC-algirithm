%MUSIC 、ROOT-MUSIC、PM-MUSIC（代价函数）、TLS-PM-MUSIC的RMSE曲线对比
%MUSIC ALOGRITHM
...
for kk=1:11
    ...
    aaa=zeros(1,2000);%用于存储估计的波达方向角度
    for k=1:2000
...
%         aaa(:,k)=estimated_source_doa;
    ...
    %
    E_source_doa=sum(aaa(1,:))/2000;
    disp('E_source_doa');
    
    RMSE_source_doa=sqrt(sum((aaa(1,:)-source_doa).^2)/2000);%均方根误差
...
%         x=A*s+(1/sqrt(2))*(randn(sensor_number,N_x)+j*randn(sensor_number,N_x));%天线阵元接收信号
    ...
    RMSE_source_doa=sqrt(sum((aaa(1,:)-source_doa).^2)/2000);%均方根求解
    disp('RMSE_source_doa');
    disp(RMSE_source_doa);
...
%利用PM传播因子进行谱峰搜索的算法的RMSE曲线（利用总体最小平方TLS准则估计的传播算子P）
clc
clear 
...
legend('MUSIC','ROOT-MUSIC','代价函数-PM-MUSIC(噪声子空间Q正交化)','TLS-PM-MUSIC(噪声子空间Q正交化)','CRB')
grid on
