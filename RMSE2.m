...
s=10.^((snr(kk)/2)/10)*exp(j*w*[0:N_x-1]);%??????? 
%x=awgn(s,snr); 
x=A*s+(1/sqrt(2))*(randn(sensor_number,N_x)+j*randn(sensor_number,N_x));%??????????????????§ß?????? 
...
%????????????????? 
E_source_doa=sum(aaa(1,:))/300;%??300????????? 
disp('E_source_doa'); 
